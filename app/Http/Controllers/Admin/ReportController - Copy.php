	...
	{ // campaing
	       ...
	       if(Request('type') AND request('type') == 'campaing')
	       {
				if(Request()->has('exportData')){
					return Excel::download(new CampaignReportExport, 'CampaignReportExport.xlsx');
				}  
				if(userRole() == 'leader')
				{
				...
				$tempOne = Contact::where(function ($q) use ($users){
					if(userRole() == 'leader'){
						...
						$q->where('projects.country_id','2');
					}else if(userRole() == 'sales admin saudi'){
						$q->where('projects.country_id','1');
				  	}
				});
				...
				$tempOne = $tempOne->groupBy('campaign')->pluck('campaign');
	   		    $campaings = Campaing::whereIn('name',$tempOne)->orderBy('id','desc')->paginate(10);
