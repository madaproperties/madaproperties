...
class ReportController extends Controller
{
	...
	public function reportUsers(){ // reportUsers
		...
		if(request()->has('users_id') && request()->has('from') && request()->has('to')){
			...
			// get logs
			// $meeting_count = Contact::whereBetween('created_at',[ $from,$to ])->where('user_id',$user_id)->whereHas('logs', function($q2) use($from,$to,$user_id) {
			// 	$q2->where('logs.type','meeting');
			// })->count();

			$activities = Log::where('user_id',$user_id)
								->whereHas('contact', function($q2) use($from,$to) {
