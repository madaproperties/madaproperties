...
class ReportController extends Controller
{
	...
	public function reportUsers(){ // reportUsers
		...
		$userReport = [];
		if(request()->has('users_id') && !empty(request('users_id')) && request()->has('from') && request()->has('to')){
		...
		}else if(request()->has('time_zone')){
			$allUsersReport = true;
			$from = date('Y-m-d 00:00:00', strtotime(Request('from')));
			$to = date('Y-m-d 23:59:59', strtotime(Request('to')));

			$user_id = User::where('time_zone','like','%'.request('time_zone').'%')->get()->pluck('id');
			if(Request('users_id') > 0){
				$user_id = Request('users_id');
			}
			$activities = Log::whereIn('user_id',$user_id)
								//->where('is_log','1')
								->whereBetween('log_date',[ $from,$to ])
								->get();
			$status_not_changed_after_1_week = Contact::select('id','user_id','status_id','status_changed_at')
        ...
        if(userRole() == 'sales admin saudi'){
			...
			$users = $users->where('time_zone','like','%'.request('time_zone').'%');
		}
    ...
    public function getUsers(Request $request)
    {
        ...
        if(userRole() == 'sales admin saudi'){
			...
			$users = $users->where('time_zone','like','%'.request('country').'%');
		}
		$users = $users->whereNotIn('email',['lead-admin-uae@madaproperties.com','lead-admin-ksa@madaproperties.com'])
		->orderBy('email')->get();
