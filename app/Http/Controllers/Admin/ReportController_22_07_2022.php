...
class ReportController extends Controller
{
	...
	public function reportUsers(){ // reportUsers
		...
		if(userRole() == 'leader'){
			...
			$users = User::where('leader',auth()->id())->OrWhere('id',auth()->id())->orderBy('email')->get();
		}
	...
	public function reportAdvanceCampaing(){ // campaing
		...
		if(Request('type') AND request('type') == 'campaing'){
			if(Request()->has('exportData')){
			return Excel::download(new CampaignReportExport, 'AdvanceCampaignReportExport_'.date('d-m-Y').'.xlsx');
			}  
