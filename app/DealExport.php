...
class DealExport implements FromQuery, WithHeadings, ShouldAutoSize, WithMapping
{
    ...
    public function map($deal): array
    {
        ...
        return [
          ...
          $deal->commission,
          number_format($deal->commission_amount),
          $deal->vat,
          number_format($deal->vat_amount),
          $deal->vat_received,
          number_format($deal->total_invoice),
          $deal->token,
          number_format($deal->down_payment),
          $deal->spa,
          date('d-m-Y',strtotime($deal->expected_date)),
          date('d-m-Y',strtotime($deal->invoice_date)),
          $agent,
          $deal->agent_commission_percent,
          number_format($deal->agent_commission_amount),
          $deal->agent_commission_received,
          $leader,
          $deal->agent_leader_commission_percent,
          number_format($deal->agent_leader_commission_amount),
          $deal->agent_leader_commission_received,
          $deal->third_party,
          number_format($deal->third_party_amount),
