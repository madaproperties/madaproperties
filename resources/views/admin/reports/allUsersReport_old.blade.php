				...
				<tbody>
					...
					@foreach($userReport as $rs)
						@php
							...
							$activities = $activities->groupBy('contact_id')->get();
						@endphp
						<tr>
							<th>{{$rs->name}}</th>
							@php
					...
					<tbody>
						@foreach($userReport as $rs)
							...
							$two_week_report = App\Contact::select('id','user_id','status_id','status_changed_at')
									->where('user_id',$rs->id)
									->whereDate('created_at', '>=', Carbon\Carbon::today()->subDays( 14 ))
									->get();
							@endphp
							...
							<tr>
								...
								@foreach($status as $state)
									<th scope="row">
										<a href="{{route('admin.home')}}?ADVANCED=search&user_id={{$rs->id}}&status_id={{$state->id}}&from={{$last14days}}">
											@php
								...
								@endforeach
								<th>
									<a href="{{route('admin.home')}}?ADVANCED=search&user_id={{$rs->id}}&status_id=&from={{$last14days}}">
									{{$finalTotal}}
								</th>
							</tr>
						@endforeach
