  							...
  							<div class="card card-custom">
  							 ...
  							 <form action="" class="form" action="get" id="search-form">
  								  <div class="card-body">
										...
										@if(userRole() != 'sales' || userRole() != 'sales admin uae' || userRole() != 'sales admin saudi')
										<div class="form-group ">
											<label class="">{{ __('site.time_zone')}}</label>
											<div class="">
												<select name="time_zone" class="form-control form-control-lg form-control-solid mb-2" onchange="getCountryUsers(this.value)">
													<option value="">{{__('site.all')}}</option>
													@foreach(timeZones() as $zone)
													<option 
													{{ request('time_zone') == $zone ? 'selected' : '' }}
...
@push('js')
<script>
function getCountryUsers(country,city ="#users"){
	let token = $('meta[name=csrf-token]').attr('content');
	let route = '{{route("admin.report.getUsers")}}';
	let cityEl = city;
	$.ajax({
		type:'GET',
		url: route,
