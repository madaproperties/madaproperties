...
$url = "https://graph.facebook.com/v14.0/$leadgen_id/?fields=campaign_name,is_organic,adset_name,ad_name,platform,field_data&access_token=EAAIO5zs15AwBAAioAQZCsrYloIiLbZBzMW85Jz7T6SxmNQAk7gsZAlfvKSgiYkHCPHcTlOytA7U0c8AOIbwHpfMNo1LEvDLslEqyxqjNU2ZAl2vnSEZAFr0jBY7KRF8AQSwnUprETLggiJfxlOQTuzNiiGEgTuIr7wtWPTqzs4orYCgtUlhzn5bDqJbzQD4kZD";
...
foreach ($lead_data as $key) {
	...
	//} elseif (($key->name == 'phone_number') || ($key->name == 'номермобильного') || ($key->name == 'numéro_de_téléphone') || ($key->name == 'номер_телефона') || ($key->name == 'رقم_الهاتف')) {
	} elseif (($key->name == 'phone_number') || ($key->name == 'номер_телефона') || ($key->name == 'numéro_de_téléphone') || ($key->name == 'رقم_الهاتف')) {
		...
		$arg["firstname"] = $key->values[0];
	} elseif (($key->name == 'mobile_number:') || ($key->name == 'номер_мобильного:') || ($key->name == '_номер_мобильного:')) {
		$arg["scound_phone"] = $key->values[0];
	}
}

$file = 'crm-integration-2.txt';
$data = file_get_contents($file);
$data .= "\n Start key names ------- \n";
$data .= json_encode($key_name);
$data .= "\n End key names ------- \n";
...
if(isset($adname_exp[3])){
	$arg["campaign_country"] = $adname_exp[3];
}
if ( $platform == 'ig') {
...
//	  "country_fromat" => 'iso',
	  ...
	  'lang' => $arg["language"],
	  'campaign_country' => isset($arg["campaign_country"]) ? $arg["campaign_country"] : '',
	  'scound_phone' => isset($arg["scound_phone"]) ? $arg["scound_phone"] : '',
  );
