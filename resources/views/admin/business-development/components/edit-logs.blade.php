    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="card card-custom">
          <!--begin::Form-->
            <form id="edit-log-{{$log->id}}-form" action="{{route('admin.business_logs.update',$log->id)}}" method="POST">
              @csrf
              @method('PUT')
        ...
        <div class="form-group  fv-plugins-icon-container">
          ...
          <div class="">
            ...
            <option value="">choose  {{$log->contact_person}}</option>
             @foreach($persons as $person)
            <option {{$log->contact_person == $person->id ? 'selected' : ''}} value="{{$person->id}}" data-select2-id="{{$person->id}}">{{$person->name}}</option>
              ...
              <div class="form-group">
                ...
                <select name="requirement_id" class="form-control" required>
                  ...
                  @foreach($requirements as $requirement)
                    <option {{$log->requirement_id == $requirement->id ? 'selected' : ''}} value="{{$requirement->id}}" data-select2-id="{{$person->id}}">{{__('site.requirement') . ' '. $z++}}</option>
                  @endforeach
                </select>
              </div>
              @endif
